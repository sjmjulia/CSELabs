                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    if(yfs->setattr(ino,YFS_SET_ATTR_SIZE,info)==yfs_client::OK)
    {
      getattr(ino,st);
      fuse_reply_attr(req, &st, 0);
    }
...
#if 1
  // Change the above line to "#if 1", and your code goes here
  if(yfs->write(ino,buf,size,off) == yfs_client::OK)
  {
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  yfs_client::inum ino;
  ret = yfs->create(parent,name,ino);
  if(ret ==  yfs_client::OK)
  {
    e->ino =ino;
    getattr(ino,e->attr);
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  yfs_client::inum ino = yfs->lookup(parent,name);
  if(ino > 0) found = true;
  else found = false;
  if (found)
  {
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  std::map<yfs_client::inum,std::string> items;
  if(yfs->readdir(ino,items) == yfs_client::OK)
  {
    std::map<yfs_client::inum,std::string>::iterator it;
    for(it = items.begin(); it != items.end(); ++it)
    {
      dirbuf_add(&b,it->second.c_str(), it->first);
    }
